...
import { ArrowRight, Calendar, MapPin, Utensils, ShoppingBag, BookOpen, Check, ChevronLeft, Gift, Sparkles, Send, Coffee } from 'lucide-react';

const MY_TELEGRAM_USERNAME = "vhanzevych"; 
...
const App: React.FC = () => {
  ...
  const finalMessage = useMemo(() => {
    const dateRange = choices.departureDate && choices.arrivalDate 
      ? `${new Date(choices.departureDate).toLocaleDateString('uk-UA')} — ${new Date(choices.arrivalDate).toLocaleDateString('uk-UA')}` 
      : 'дати не обрано';
      
    return `Пане чарівник, я обрала собі:

🗓️ Період поїздки: ${dateRange}
🏛️ Вибраний район: ${selectedDistrict?.name || 'не обрано'}
🥂 Святкова вечеря: ${selectedRestaurant?.name || 'не обрано'}

Вже чекаю своєї подорожі! ✨`;
  }, [choices, selectedDistrict, selectedRestaurant]);

  const telegramDirectChatLink = useMemo(() => {
    return `https://t.me/${MY_TELEGRAM_USERNAME}?text=${encodeURIComponent(finalMessage)}`;
  }, [finalMessage]);
          ...
          {(() => {
            switch (step) {
                ...
                return (
                  <div className="relative flex flex-col items-center justify-center min-h-screen text-center px-4 animate-in fade-in zoom-in duration-1000 overflow-hidden">
                    <div className="relative z-10 flex flex-col items-center justify-center">
                      ...
                      <button onClick={nextStep} className="px-12 py-5 bg-stone-900 text-white rounded-full hover:bg-stone-800 transition-all tracking-widest text-sm uppercase flex items-center gap-3 shadow-2xl hover:scale-105 active:scale-95 group mx-auto font-bold">
                ...
                return (
                  <div className="flex flex-col items-center justify-center min-h-[80vh] text-center px-4 animate-in fade-in duration-1000 relative">
                    <div className="space-y-6 max-w-4xl">
                      <p className="text-xl md:text-3xl font-serif text-stone-600 leading-relaxed font-light">Я пам’ятаю, ти казала, що дуже хочеш в одне красиве місце.</p>
                      <p className="text-xl md:text-3xl font-serif text-stone-600 leading-relaxed font-light">І я подумав, що бажання мають збуватися тихо і красиво...</p>
                      <h2 className="text-4xl md:text-6xl font-serif text-stone-900 italic font-bold pt-6 animate-pulse">Особливо твої</h2>
                    </div>
                    <button onClick={nextStep} className="mt-20 px-16 py-6 bg-stone-900 text-white rounded-full hover:bg-stone-800 transition-all tracking-widest text-sm uppercase font-bold flex items-center gap-3 shadow-xl hover:scale-105 active:scale-95 group">
              ...
              case 'quote':
                return (
                  <div className="flex flex-col items-center justify-center min-h-[80vh] text-center px-4 animate-in fade-in duration-1000 relative">
                    <h2 className="text-xl md:text-3xl mb-16 font-serif italic text-stone-800 leading-[1.8] max-w-4xl px-6">Італія — це місце, яке стає частиною<br />спогадів на все життя.</h2>
                    <button onClick={nextStep} className="px-16 py-6 border-2 border-stone-900 text-stone-900 rounded-full hover:bg-stone-900 hover:text-white transition-all tracking-widest text-sm uppercase font-bold flex items-center gap-3 shadow-sm hover:shadow-xl hover:scale-105 active:scale-95 group">
                ...
                return (
                  <div className="max-w-4xl mx-auto py-12 px-4 animate-in slide-in-from-bottom-4 duration-700 text-center relative">
                    <div className="mb-16">
                      ...
                      <p className="text-stone-900 text-xl md:text-2xl font-serif leading-relaxed max-w-3xl mx-auto font-medium">І хочу, щоб все складалось під твій ритм, твій настрій і твої бажання. Перше, що тобі потрібно — це обрати свій тиждень у Вічному Місті.</p>
                    </div>
                    <div className="flex flex-col md:flex-row justify-center gap-8 items-center max-w-2xl mx-auto">
                      <div className="flex-1 w-full text-left">
                        ...
                        <div className="relative group">
                          <input type="date" min={new Date().toISOString().split('T')[0]} value={choices.departureDate} onChange={(e) => setChoices({ ...choices, departureDate: e.target.value })} className="w-full p-5 bg-white border border-stone-100 rounded-2xl shadow-sm focus:ring-2 focus:ring-stone-800/5 focus:border-stone-800 outline-none transition-all font-serif text-lg cursor-pointer" />
                      ...
                      <div className="flex-1 w-full text-left">
                        ...
                        <div className="relative group">
                          <input type="date" min={choices.departureDate || new Date().toISOString().split('T')[0]} max={maxArrivalDate} value={choices.arrivalDate} disabled={!choices.departureDate} onChange={(e) => setChoices({ ...choices, arrivalDate: e.target.value })} className="w-full p-5 bg-white border border-stone-100 rounded-2xl shadow-sm focus:ring-2 focus:ring-stone-800/5 focus:border-stone-800 outline-none transition-all font-serif text-lg cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed" />
                          <Calendar size={18} className="absolute right-4 top-1/2 -translate-y-1/2 text-stone-300 group-hover:text-stone-800 transition-colors pointer-events-none" />
                        </div>
                      </div>
                    </div>
                    {choices.departureDate && choices.arrivalDate && (
                      <button onClick={nextStep} className="mt-16 px-16 py-5 bg-stone-900 text-white rounded-full hover:bg-stone-800 transition-all flex items-center gap-3 mx-auto shadow-xl hover:scale-105 font-bold">Продовжити <ArrowRight size={18} /></button>
                    )}
                  </div>
                );
              case 'stay':
                return (
                  <div className="max-w-6xl mx-auto pt-4 pb-12 px-4 animate-in slide-in-from-bottom-4 duration-700 relative">
                    <div className="text-center mb-10 pt-4">
                      <h2 className="text-4xl md:text-5xl font-serif mb-6 text-stone-800 leading-tight">Місто відчувається по-різному<br /><span className="italic">залежно від того, де ти живеш</span></h2>
                      <p className="text-stone-500 max-w-xl mx-auto text-lg font-light leading-relaxed">Обери район,<br />у якому тобі захочеться прокидатися<br />і повертатися ввечері</p>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                      {DISTRICTS.map((district) => (
                        <div key={district.id} onClick={() => setChoices({ ...choices, district: district.id })} className={`group cursor-pointer rounded-2xl overflow-hidden border-2 transition-all duration-500 relative ${choices.district === district.id ? 'border-stone-800 shadow-2xl scale-[1.02] bg-white' : 'border-transparent shadow-sm opacity-80 hover:opacity-100 bg-white/50'}`}>
                          ...
                          <div className="p-8">
                            <h3 className="text-2xl font-serif mb-3 text-stone-800 font-bold">{district.name}</h3>
                            ...
                            {choices.district === district.id && (
                              <div className="mt-6 flex justify-center animate-in fade-in slide-in-from-bottom-2"><Check size={24} className="text-green-600" /></div>
                            )}
                          </div>
                        </div>
                      ))}
                    ...
                    {choices.district && (
                      <button onClick={nextStep} className="mt-16 px-12 py-4 bg-stone-900 text-white rounded-full hover:bg-stone-800 transition-all flex items-center gap-2 mx-auto shadow-lg font-bold">Обираємо ресторан <ArrowRight size={18} /></button>
                    )}
                  </div>
                );
              case 'dine':
                return (
                  <div className="max-w-6xl mx-auto pt-0 pb-12 px-4 animate-in slide-in-from-bottom-4 duration-700 relative">
                    <div className="text-center mb-10 pt-16">
                      ...
                      <p className="text-stone-600 max-w-2xl mx-auto text-lg font-light leading-relaxed">Я хочу щоб твоя подорож була смачною, тому в поїздку входить вечеря на двох в одному з цих особливих місць. Вибери те, що тобі до душі!</p>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                      {RESTAURANTS.map((rest) => (
                        <div key={rest.id} onClick={() => setChoices({ ...choices, restaurant: rest.id })} className={`group cursor-pointer rounded-2xl overflow-hidden border-2 transition-all duration-500 bg-white ${choices.restaurant === rest.id ? 'border-stone-800 shadow-2xl scale-[1.02]' : 'border-transparent shadow-sm opacity-80 hover:opacity-100'}`}>
                          <div className="h-64 overflow-hidden relative">
                          ...
                          <div className="p-8">
                            <h3 className="text-2xl font-serif mb-3 text-stone-800 font-bold">{rest.name}</h3>
                            <p className="text-stone-500 text-sm leading-relaxed font-light mb-4">{rest.description}</p>
                            {choices.restaurant === rest.id && (
                              <div className="mt-6 flex justify-center animate-in fade-in slide-in-from-top-2"><Check size={24} className="text-green-600" /></div>
                            )}
                          </div>
                        </div>
                      ))}
                    ...
                    {choices.restaurant && (
                      <button onClick={nextStep} className="mt-16 px-12 py-4 bg-stone-900 text-white rounded-full hover:bg-stone-800 transition-all flex items-center gap-2 mx-auto shadow-lg font-bold">Пам'ятки міста <ArrowRight size={18} /></button>
                    )}
                  </div>
                );
              case 'explore':
                return (
                  <div className="max-w-5xl mx-auto py-12 px-4 animate-in slide-in-from-bottom-4 duration-700 relative">
                    <div className="text-center mt-12 mb-20">
                      ...
                      <p className="text-stone-800 max-w-2xl mx-auto text-lg font-bold leading-relaxed">Я вибрав ці місця не за списками з інтернету, а за відчуттям. Це ті куточки міста, де хочеться зупинитись, подивитись навколо і запамʼятати момент.</p>
                    </div>
                      ...
                      {ATMOSPHERIC_PLACES.map((place, index) => (
                        <div key={place.name} className="group flex flex-col space-y-4">
                          ...
                          <div className="space-y-2 px-2">
                            ...
                            <p className="text-stone-700 font-bold leading-relaxed text-sm italic">«{place.description}»</p>
                          </div>
                        </div>
                      ))}
                    </div>
                    <div className="relative pt-32 pb-16 border-t border-stone-100">
                      <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white px-8 text-stone-400"><Sparkles size={24} /></div>
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
                        ...
                        <div className="lg:col-span-6 order-2 flex flex-col gap-20 py-4">
                          <div className="grid grid-cols-1 gap-12">
                            <div>
                              <div className="flex items-center gap-4 mb-10 text-stone-600"><ShoppingBag size={20} /><span className="uppercase tracking-[0.5em] text-[10px] font-bold">Магазини з душею</span></div>
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-10">
                                {SLOW_WALKS.shops.map(item => (
                                  <div key={item.name} className="space-y-3">
                                    <h4 className="font-serif text-2xl text-stone-900 font-bold">{item.name}</h4>
                                    <p className="text-xs text-stone-700 font-bold uppercase tracking-widest">{item.note}</p>
                                  </div>
                                ))}
                              </div>
                            </div>
                            <div>
                              <div className="flex items-center gap-4 mb-10 text-stone-600"><Coffee size={20} /><span className="uppercase tracking-[0.5em] text-[10px] font-bold">Торгові центри</span></div>
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-10">
                    ...
                    <button onClick={nextStep} className="mt-24 px-16 py-6 bg-stone-900 text-white rounded-full hover:bg-stone-800 transition-all flex items-center gap-3 mx-auto shadow-2xl hover:scale-105 active:scale-95 font-bold uppercase tracking-widest text-[10px]">
              ...
              case 'commitment':
                return (
                  <div className="max-w-4xl mx-auto py-12 px-4 animate-in fade-in duration-1000 text-center relative flex flex-col items-center justify-center min-h-[70vh]">
                    <div className="mb-12 text-stone-300"><Sparkles size={64} /></div>
                    <h2 className="text-3xl md:text-5xl font-serif text-stone-900 mb-12 leading-tight font-bold">Ти щойно створила свою подорож до Риму.<br /><span className="italic font-normal text-stone-700">Дати, ритм, простір — усе так, як відчувається правильно для тебе.</span></h2>
                    <p className="text-xl md:text-3xl text-stone-800 font-serif italic mb-16 max-w-2xl mx-auto">Все інше я беру на себе.</p>
                    <button onClick={nextStep} className="px-16 py-6 bg-stone-900 text-white rounded-full hover:bg-stone-800 transition-all flex items-center gap-3 shadow-2xl hover:scale-105 active:scale-95 font-bold uppercase tracking-widest text-xs mx-auto">Переглянути підсумок <ArrowRight size={18} /></button>
                  </div>
                );
              case 'summary':
                return (
                  <div className="max-w-4xl mx-auto py-16 px-4 animate-in zoom-in duration-700 text-center relative">
                    ...
                    <div className="bg-white p-12 md:p-16 rounded-[3rem] shadow-2xl border border-stone-50 relative overflow-hidden text-left mb-16">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-16 relative z-10">
                        ...
                        <div className="flex flex-col justify-center">
                          <img src={selectedDistrict?.image || 'https://images.unsplash.com/photo-1552832230-c0197dd311b5?auto=format&fit=crop&w=800&q=80'} alt="Rome Destination" className="w-full h-64 object-cover rounded-[2rem] shadow-xl border-8 border-white relative z-10" />
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col items-center gap-4 print:hidden">
                      <div className="flex justify-center w-full">
                        <a 
                          href={telegramDirectChatLink}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="px-16 py-6 bg-sky-500 text-white rounded-full hover:bg-sky-600 transition-all font-bold uppercase tracking-[0.2em] text-[10px] shadow-2xl flex items-center justify-center gap-3 group hover:scale-105 active:scale-95"
                        >
                          <Send size={18} className="group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" /> Надіслати бажання чарівнику
                        </a>
                      </div>
                      <p className="text-stone-400 text-[10px] uppercase tracking-widest font-bold mt-4">Натисни кнопку, щоб відправити вибір у чат</p>
                    </div>
                  </div>
                );
              default:
      ...
      {step !== 'welcome' && step !== 'memory' && step !== 'quote' && step !== 'date' && step !== 'summary' && (
        <div className="fixed bottom-12 left-1/2 -translate-x-1/2 flex gap-4 px-8 py-4 bg-white/70 backdrop-blur-xl rounded-full shadow-2xl border border-white/50 print:hidden z-50">
          {sequence.filter(s => !['welcome', 'memory', 'quote', 'date', 'summary'].includes(s)).map((s) => (
            <div key={s} className={`h-1.5 rounded-full transition-all duration-700 ${step === s ? 'bg-stone-800 w-12' : 'bg-stone-200 w-2'}`} />
          ))}
        </div>
      )}
    </div>
